...
def assert_article_structure(
    article: Dict[str, Any], required_fields: Optional[List[str]] = None
) -> None:
    """
...
def assert_article_metadata(
    ...
    if expected_url:
        assert (
            article["url"] == expected_url
        ), f"URL mismatch: expected '{expected_url}', got '{article['url']}'"

    if expected_source:
        assert (
            article["source_api"] == expected_source
        ), f"Source mismatch: expected '{expected_source}', got '{article['source_api']}'"


def assert_article_processing_status(
    article: Dict[str, Any], expected_status: str, check_content: bool = True
) -> None:
    """
    ...
    assert (
        article["processing_status"] == expected_status
    ...
    if check_content and expected_status == "SUCCESS":
        assert (
            "processed_content" in article
        ), "Successful processing should have processed_content"
...
def assert_article_embedding_status(
    article: Dict[str, Any], expected_status: str, check_model: bool = True
) -> None:
    """
    ...
    assert (
        article["embedding_status"] == expected_status
    ...
    if check_model and expected_status == "SUCCESS":
        assert (
            "embedding_model" in article
        ), "Successful embedding should have embedding_model"
        assert article["embedding_model"], "Embedding model should not be empty"


def assert_database_statistics(
    stats: Dict[str, Any],
    ...
    assert (
        stats["total_articles"] >= min_total_articles
    ), f"Expected at least {min_total_articles} articles, got {stats['total_articles']}"

    if expected_statuses:
        for status in expected_statuses:
            assert (
                status in stats["text_processing_status"]
            ), f"Expected status '{status}' in text_processing_status"
            assert (
                status in stats["embedding_status"]
            ), f"Expected status '{status}' in embedding_status"


def assert_chroma_collection_status(
    status: Dict[str, Any], min_chunks: int = 0, min_articles: int = 0
) -> None:
    """
    ...
    if status["total_chunks"] == 0:
        ...
        assert (
            status["unique_articles"] == 0
        ), "Empty collection should have 0 unique articles"


def assert_chroma_query_results(
    ...
    for i, result in enumerate(results):
        ...
        assert (
            "article_url_hash" in metadata
        ), f"Result {i} metadata missing article_url_hash"
...
def assert_api_response(
    ...
    if expected_success:
        assert (
            len(response["articles"]) >= min_articles
...
def assert_orchestrator_result(
    result: Dict[str, Any], expected_status: str, expected_fields: List[str] = None
) -> None:
    """
...
def assert_date_range_valid(date_range: Dict[str, str]) -> None:
    ...
    try:
        earliest = datetime.fromisoformat(
            date_range["earliest_article"].replace("Z", "+00:00")
        )
        latest = datetime.fromisoformat(
            date_range["latest_article"].replace("Z", "+00:00")
        )
...
def assert_search_results(
    ...
    for i, result in enumerate(results):
        ...
        if check_relevance:
            ...
            has_relevance = any(
                term in content_lower or term in title_lower for term in query_terms
            )
            assert has_relevance, f"Result {i} appears irrelevant to query '{query}'"


def assert_search_result_structure(
    result: Dict[str, Any], has_rerank_score: bool = None
) -> None:
    """
    ...
    for field in required_fields:
        ...
        ), f"Search result field '{field}' cannot be None"
    ...
    ), f"similarity_score {similarity_score} outside valid range [0, 1]"
    ...
    if has_rerank_score:
        ...
        ), "Expected rerank_score field in reranked result"
        rerank_score = result["rerank_score"]
        assert isinstance(rerank_score, (int, float)), "rerank_score must be numeric"
        assert (
            0 <= rerank_score <= 1
        ), f"rerank_score {rerank_score} outside valid range [0, 1]"
    ...
    assert isinstance(
        result["processed_content"], str
    ), "processed_content must be a string"
    assert (
        len(result["processed_content"].strip()) > 0
    ), "processed_content cannot be empty"
    ...
    try:
        ...
        raise AssertionError(
            f"published_at '{result['published_at']}' is not a valid ISO datetime string"
        )
