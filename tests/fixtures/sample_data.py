...
class ArticleFactory:
    ...
    def create_article(
        ...
        **kwargs,
        ...
        if published_at is None:
            ...
            pub_time = datetime.now(timezone.utc) - timedelta(
        ...
        # Generate content
        content_fragments = random.sample(
            cls.SAMPLE_CONTENT_FRAGMENTS, random.randint(2, 4)
        )
    ...
    @classmethod
    def create_articles_batch(cls, count: int, **common_kwargs) -> List[Dict[str, Any]]:
        """
    ...
    def create_eodhd_articles_batch(
        cls, count: int, symbols: Optional[List[str]] = None, **common_kwargs
    ) -> List[Dict[str, Any]]:
        """
...
class ChunkFactory:
    ...
    @classmethod
    def create_chunks(
        cls, count: int, custom_content: Optional[List[str]] = None
    ) -> List[str]:
        """
    ...
    @classmethod
    def create_embeddings(cls, count: int, dimension: int = 768) -> List[List[float]]:
        """
...
class EODHDResponseFactory:
    ...
    def create_response(
        ...
        """
        if articles is None:
            articles = ArticleFactory.create_eodhd_articles_batch(
    ...
    def create_success_response(
        cls, articles: Optional[List[Dict[str, Any]]] = None, count: int = 2
    ) -> Dict[str, Any]:
        """
    ...
    def create_error_response(
        cls, status_code: int = 400, error_message: str = "API request failed"
    ) -> Dict[str, Any]:
        """
...
class ChromaResultsFactory:
    ...
    def create_query_results(
        ...
        for i in range(count):
            ...
            if i % 2 == 0:
                result["metadata"]["source_query_tag"] = random.choice(
                    ArticleFactory.TAGS
                )
            else:
                result["metadata"]["source_query_symbol"] = random.choice(
                    ArticleFactory.SYMBOLS
                )
...
class EmbeddingFactory:
    ...
    @classmethod
    def create_normalized_embeddings(
        cls, count: int, dimension: int = 768
    ) -> List[np.ndarray]:
        """
...
def create_test_normalized_embeddings(
    count: int, dimension: int = 768
) -> List[np.ndarray]:
