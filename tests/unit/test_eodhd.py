...
class TestEODHDClientFetchNews:
    ...
    def test_fetch_news_no_parameters_raises_error(self, client):
        ...
        with pytest.raises(ValueError, match="Either 'tag' or 'symbol' parameter is required"):
            client.fetch_news()
...
class TestEODHDClientRetryLogic:
    ...
    @patch("requests.get")
    @patch("time.sleep")  # Mock sleep to speed up tests
    def test_retry_on_temporary_failure(self, mock_sleep, mock_get, client_with_retries):
