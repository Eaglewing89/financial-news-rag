...
class TestChromaDBManagerCollectionManagement:
    ...
    def test_delete_embeddings_by_article(self, isolated_chroma_manager):
        ...
        success = isolated_chroma_manager.delete_embeddings_by_article(first_article["url_hash"])
...
class TestChromaDBManagerDateRangeOperations:
    ...
    @pytest.fixture
    def populated_chroma_manager(self, chroma_manager, sample_articles_with_dates, sample_dates):
        """Populate chroma_manager with sample articles with different dates."""
        for i, (article, date) in enumerate(zip(sample_articles_with_dates, sample_dates)):
    ...
    def test_query_embeddings_invalid_date_filters(self, chroma_manager):
        ...
        results = chroma_manager.query_embeddings(
            query_embedding=query_embedding,
            n_results=10,
            to_date_str="not-a-date"
        )
        # Should still return results, ignoring invalid filter
        assert len(results) == 1
    
    def test_get_article_hashes_by_date_range_both_timestamps(self, populated_chroma_manager):
    ...
    def test_get_article_hashes_by_date_range_single_timestamp(self, isolated_chroma_manager):
...
class TestChromaDBManagerArticleChunks:
    ...
    def test_handling_duplicate_chunk_ids_with_article_chunks(self, isolated_chroma_manager):
