...
from tests.helpers.assertions import (
    assert_search_result_structure,
)
...
class BaseTestFinancialNewsRAGIntegration:
    ...
    def setup_integration_environment(self, temp_directory):
        ...
        self.gemini_embedding_patcher = patch(
            "financial_news_rag.embeddings.genai.Client"
        )
    ...
    def orchestrator(self, mock_embedding_values):
        ...
        orchestrator.embeddings_generator.client.models.embed_content = (
            mock_embed_content
        )
...
class TestOrchInitAndConfig(BaseTestFinancialNewsRAGIntegration):
    ...
    def test_configuration_inheritance_integration(self, orchestrator):
        ...
        assert orchestrator.chroma_manager.persist_directory == self.test_chroma_dir
        assert (
            orchestrator.chroma_manager.collection_name == "test_integration_collection"
        )
    ...
    def test_constructor_error_handling_integration(self):
        ...
        with patch.dict(
            ...
            with pytest.raises(ValueError, match="EODHD API key not provided"):
                FinancialNewsRAG(
                    db_path=self.test_db_path, chroma_persist_dir=self.test_chroma_dir
                )
...
class TestWorkflowIntegration(BaseTestFinancialNewsRAGIntegration):
    ...
    def test_complete_rag_workflow_integration(self, orchestrator):
        ...
        assert (
            db_status_after_processing["text_processing_status"].get("PENDING", 0) == 0
        )
        ...
        search_results = orchestrator.search_articles(
            query="Apple earnings technology", n_results=2, rerank=False
        )
        ...
        reranked_results = orchestrator.search_articles(
            query="Microsoft cloud services", n_results=2, rerank=True
        )
...
class TestComponentInteraction(BaseTestFinancialNewsRAGIntegration):
    ...
    def test_component_interaction_integration(self, orchestrator):
        ...
        search_results = orchestrator.search_articles(
            "Microsoft", n_results=1, rerank=True
        )
...
class TestErrorHandling(BaseTestFinancialNewsRAGIntegration):
    ...
    def test_error_handling_integration(self, orchestrator):
        ...
        with pytest.raises(
            ValueError, match="tag and symbol parameters are mutually exclusive"
        ):
    ...
    def test_process_articles_exception_handling_integration(self, orchestrator):
        ...
        with patch.object(
            orchestrator.text_processor, "process_and_validate_content"
        ) as mock_process:
            ...
            )  # Overall operation succeeds but individual articles fail
        ...
        with patch.object(
            orchestrator.article_manager, "get_articles_by_processing_status"
        ) as mock_get:
    ...
    def test_embed_articles_exception_handling_integration(self, orchestrator):
        ...
        with patch.object(
            orchestrator.article_manager, "get_processed_articles_for_embedding"
        ) as mock_get:
        ...
        with patch.object(
            orchestrator.text_processor, "split_into_chunks"
        ) as mock_split:
        ...
        with patch.object(
            orchestrator.embeddings_generator, "generate_and_verify_embeddings"
        ) as mock_embed:
        ...
        with patch.object(
            orchestrator.chroma_manager, "add_article_chunks"
        ) as mock_add:
    ...
    def test_database_status_exception_handling_integration(self, orchestrator):
        ...
        with patch.object(
            orchestrator.article_manager, "get_database_statistics"
        ) as mock_status:
        ...
        with patch.object(
            orchestrator.chroma_manager, "get_collection_status"
        ) as mock_status:
    ...
    def test_search_exception_handling_integration(self, orchestrator):
        ...
        with patch.object(
            orchestrator.embeddings_generator, "generate_embeddings"
        ) as mock_embed:
        ...
        with patch.object(
            orchestrator.chroma_manager, "query_embeddings"
        ) as mock_query:
        ...
        with patch.object(
            orchestrator.article_manager, "get_article_by_hash"
        ) as mock_get:
    ...
    def test_delete_operations_comprehensive_integration(self, orchestrator):
        ...
        with patch.object(
            orchestrator.chroma_manager, "get_article_hashes_by_date_range"
        ) as mock_get_old:
            mock_get_old.return_value = ["old_hash_1", "old_hash_2"]

            # Test successful deletion
            with patch.object(
                orchestrator.chroma_manager, "delete_embeddings_by_article"
            ) as mock_del_chroma, patch.object(
                orchestrator.article_manager, "delete_article_by_hash"
            ) as mock_del_sqlite:
            ...
            with patch.object(
                ...
                assert (
                    result["status"] == "SUCCESS"
                )  # Still success if SQLite deletion worked
                assert result["deleted_from_sqlite"] == 2
                assert result["deleted_from_chroma"] == 0
    ...
    def test_resource_cleanup_error_handling_integration(self, orchestrator):
        ...
        with patch.object(
            orchestrator.article_manager, "close_connection"
        ) as mock_close_sqlite:
        ...
        with patch.object(
            orchestrator.chroma_manager, "close_connection"
        ) as mock_close_chroma:
...
class TestSearchFunctionality(BaseTestFinancialNewsRAGIntegration):
    ...
    def test_search_functionality_integration(self, orchestrator):
        ...
        no_results = orchestrator.search_articles(
            "completely_irrelevant_query", n_results=5
        )
...
class TestPerformanceAndEdgeCases(BaseTestFinancialNewsRAGIntegration):
    ...
    def test_performance_and_scalability_integration(self, orchestrator):
        ...
        process_result = orchestrator.process_articles_by_status(
            status="PENDING", limit=10
        )
        assert process_result["articles_processed"] == 2

        # Embed all articles in batch
        embedding_result = orchestrator.embed_processed_articles(
            status="PENDING", limit=10
        )
        assert embedding_result["articles_embedding_succeeded"] == 2
        ...
        assert (
            final_vector_status["total_chunks"] >= 2
        )  # At least one chunk per article
