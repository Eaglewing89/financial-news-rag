...
from financial_news_rag.utils import get_utc_now, parse_iso_date_string
...
class EODHDClient:
    ...
    def fetch_news(
        ...
        # Fetch with retry
        raw_articles = self._fetch_with_retry(
            self.api_url, params, max_retries_to_use, backoff_factor_to_use
        )
        ...
        for article in raw_articles.get("articles", []):
            ...
            if "t" in params:
                normalized_article = self._normalize_article(
                    article, query_type="tag", query_value=params["t"]
                )
            elif "s" in params:
                normalized_article = self._normalize_article(
                    article, query_type="symbol", query_value=params["s"]
                )
            else:
    ...
    def _fetch_with_retry(
        self, url: str, params: Dict, max_retries: int, backoff_factor: float
    ) -> Dict:
        """
            ...
            try:
                ...
                if isinstance(data, dict) and "code" in data and "message" in data:
                    result["error_message"] = (
                        f"EODHD API Error: {data['message']} (Code: {data['code']})"
                    )
    ...
    def _normalize_article(
        ...
        if parsed_dt:
            ...
            logger.warning(
                f"Could not parse date string: {raw_date_str}. Falling back to current UTC time."
            )
