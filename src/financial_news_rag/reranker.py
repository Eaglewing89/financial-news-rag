...
class ReRanker:
    ...
    def _assess_article_relevance(
        self, query: str, article_content: str, url_hash: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                ...
                if score_match:
                    ...
                    logger.error(
                        f"Failed to parse relevance score from response: {response_text}"
                    )
                    return {"id": url_hash, "score": 0.0}
        ...
        try:
            ...
            for article in reranked_articles:
                if "url_hash" not in article or "processed_content" not in article:
                    logger.warning(
                        f"Article missing required fields: {article.get('url_hash', 'unknown')}"
                    )
