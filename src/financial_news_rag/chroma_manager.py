...
class ChromaDBManager:
    ...
    def _initialize_client_and_collection(self) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Created new collection: {self.collection_name} with dimension {self.embedding_dimension}"
                )
            ...
            if from_date_str:
                try:
                    ...
                    if from_timestamp is not None:
                        ...
                        logger.info(
                            f"Filtering articles published on or after {from_date_str} (timestamp {from_timestamp})"
                        )
                    else:
                        logger.warning(
                            f"Failed to parse from_date_str '{from_date_str}'"
                        )
                except Exception as e:
                    logger.warning(
                        f"Error processing from_date_str '{from_date_str}': {e}"
                    )
            ...
            if to_date_str:
                try:
                    ...
                    if to_timestamp is not None:
                        ...
                        logger.info(
                            f"Filtering articles published on or before {to_date_str} (timestamp {to_timestamp})"
                        )
                    else:
        ...
        try:
            ...
            if not is_empty:
                ...
                all_metadatas = self.collection.get(
                    include=["metadatas"], limit=count  # Get all entries
                )
    ...
    def delete_embeddings_by_article(self, article_url_hash: str) -> bool:
        ...
        try:
            ...
            self.collection.delete(where={"article_url_hash": article_url_hash})
            logger.info(
                f"Successfully deleted embeddings for article {article_url_hash}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error deleting embeddings for article {article_url_hash}: {e}"
            )
            return False
    ...
    def add_article_chunks(
        ...
        try:
            ...
            for i, (chunk_text, chunk_vector) in enumerate(
                ...
                if published_at_str is not None:
                    try:
                        ...
                        logger.warning(
                            f"Failed to convert published_at date '{published_at_str}' to timestamp: {e}"
                        )

                # Add additional metadata if available
                if article_data.get("source_query_tag"):
                    current_metadata["source_query_tag"] = article_data.get(
                        "source_query_tag"
                    )

                if article_data.get("source_query_symbol"):
                    current_metadata["source_query_symbol"] = article_data.get(
                        "source_query_symbol"
                    )
            ...
            if not ids:
                logger.warning(
                    f"No valid chunk embeddings to add for article {article_url_hash}"
                )
                return False
            ...
            logger.info(
                f"Successfully upserted {len(ids)} chunk embeddings for article {article_url_hash}"
            )
            return True

        except Exception as e:
            logger.error(
                f"Error upserting embeddings for article {article_url_hash}: {e}"
            )
            return False
    ...
    def get_article_hashes_by_date_range(
        ...
        try:
            ...
            if older_than_timestamp is None and newer_than_timestamp is None:
                logger.warning(
                    "No timestamp criteria provided for get_article_hashes_by_date_range. Returning empty list."
                )
                return []
