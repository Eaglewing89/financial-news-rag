...
class FinancialNewsRAG:
    ...
    def __init__(
        ...
        chroma_persist_dir_to_use = (
            chroma_persist_dir or self.config.chroma_default_persist_directory
        )
        chroma_collection_name_to_use = (
            chroma_collection_name or self.config.chroma_default_collection_name
        )
        max_tokens_per_chunk_to_use = (
            max_tokens_per_chunk or self.config.textprocessor_max_tokens_per_chunk
        )
    ...
    def fetch_and_store_articles(
        ...
        if tag and symbol:
            raise ValueError(
                "tag and symbol parameters are mutually exclusive - provide only one"
            )
        ...
        try:
            ...
            # Fetch by symbol
                ...
                try:
                    ...
                    stored_count = 0
                    if success and fetched_articles:
                        stored_count = self.article_manager.store_articles(
                            fetched_articles
                        )
    ...
    def process_articles_by_status(
        ...
        try:
            ...
            for article in articles:
                ...
                try:
                    ...
                    validation_result = (
                        self.text_processor.process_and_validate_content(
                    ...
                    results["articles_failed"] += 1
                    results["errors"].append(
                        f"Error processing article {url_hash}: {str(e_article)}"
                    )
    ...
    def embed_processed_articles(
        ...
        try:
            ...
            articles_for_embedding = (
                self.article_manager.get_processed_articles_for_embedding(
                    status=status, limit=limit
                )
            )
            logger.info(
            ...
            for article in articles_for_embedding:
                ...
                try:
                    ...
                    embedding_result = (
                        self.embeddings_generator.generate_and_verify_embeddings(chunks)
                    )
                    ...
                    storage_success = self.chroma_manager.add_article_chunks(
                        url_hash, chunks, chunk_embeddings, article_data_for_chroma
                    )
                    ...
                    results["articles_failed"] += 1
                    results["errors"].append(
                        f"Error {action} article {url_hash}: {str(e)}"
                    )
        ...
        try:
            ...
            if sort_by_metadata:
                for key, value in sort_by_metadata.items():
                    ...
                    if key == "published_at_timestamp" and value.lower() in [
                        "desc",
                        "asc",
                    ]:
            ...
            chroma_results = self.chroma_manager.query_embeddings(
                ...
                n_results=(
                    n_results if not rerank else max(n_results * 2, 10)
            ...
            for result in chroma_results:
                ...
                if url_hash:
                    ...
                    if (
                        url_hash not in article_scores
                        or similarity > article_scores[url_hash]
                    ):
        ...
        try:
            ...
            for url_hash in article_hashes:
                try:
                    ...
                    chroma_deleted = self.chroma_manager.delete_embeddings_by_article(
                        url_hash
                    )
                    if chroma_deleted:
                    ...
                    # Delete from SQLite
                    sqlite_deleted = self.article_manager.delete_article_by_hash(
                        url_hash
                    )
                    if sqlite_deleted:
